...
public class StockOpinionService {
    ...
    public void collectDailyData() {
        ...
        Set<String> tickers = redisTemplate.opsForZSet().range(POST_COUNT_KEY, 0, -1);
        for (String ticker : tickers) {
            try {
                int postCount = getRealtimePostCount(ticker);
                int bullishCnt = getRealtimeBullishOpinion(ticker);
                int bearishCnt = getRealtimeBearishOpinion(ticker);
                StockTickers stockTickers = stockSearchService.getStockByTicker(ticker)
                        .orElseThrow(() -> new IllegalArgumentException("해당 stockTicker 객체를 찾지 못했습니다."));
